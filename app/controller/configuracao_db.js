	...
	tbl_configuracoes.findAll()
		.then((configuracoes) => {
			if ((configuracoes == null) || (configuracoes == undefined) || (configuracoes.length == 0)) {
				res.status(404)
					.send(util.response("Not Found", 404, "Banco não encontrado", "api/configuracoes", "GET", null))
			}
			else {
				res.status(200)
...
const buscarUmaConfiguracao = (req, res, next) => {
	tbl_configuracoes.findAll({
		attributes: {
			exclude: ['db_username']
		},
		where: {
			db_database: req.params.database
		}
	})
		.then((configuracoes) => {
			if ((configuracoes == null) || (configuracoes == undefined) || (configuracoes.length == 0)) {
